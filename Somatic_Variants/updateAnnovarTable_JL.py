...
#print chr name, chr pos, Ref, Alt etc from input CSV file
	...
	#print len(somaticVariants)
	#print somaticVariants
	#print type(somaticVariants[0])
	#print type(somaticVariants)
	#if type(somaticVariants[0]) is not numpy.ndarray:
		# if there is only one variant listed in the file, then the type will not be an nparray.
	#	outputCSV.write('\t'.join([somaticVariants[0], somaticVariants[1], somaticVariants[2], somaticVariants[3], somaticVariants[4], somaticVariants[5], somaticVariants[6], somaticVariants[7], somaticVariants[8], somaticVariants[9], somaticVariants[10], somaticVariants[11]]) + '\t')
	#else:
