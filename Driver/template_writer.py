...
class TemplateWriter:
    ...
    def __writeQCSampleTemplate(self, job, fileHandle):
        if 'qc_scripts' in job:
            softwareDirectory = job['qc_scripts']
        else:
            softwareDirectory = "%s/scripts/QC"%(self.__softwareDirectory)
		# the settings should all be contained within the json file.
        fileHandle.write('python2.7 %s/QC_sample.py --json %s -e \n'%(softwareDirectory, job['json_file']))
