...
class JobManager:
	...
	def __init__(self, softwareDirectory, sampleDirectory, jobFilters, QCSettings, sendEmail):
	...
	def manageJob(self, jobFile, baseDir):
		...
		templateWriter = TemplateWriter(outputFolder, self.__softwareDirectory, self.__QCSettings, self.__sendEmail)
		...
		if 'software_directory' not in fileData['analysis'] or self.__softwareDirectory != fileData['analysis']['software_directory']:
			fileData['analysis']['software_directory'] = self.__softwareDirectory
...
if (__name__ == "__main__"):
	...
	parser.add_option('-S', '--software_dir', dest='software_dir', default="/rawdata/scripts/TRI_Scripts", help='The software root directory. [default: %default]')
	...
	parser.add_option('-q', '--qc_settings', dest="qc_settings", help="Extra settings to use when running the QC scripts")
	parser.add_option('-e', '--email', dest="email", action='store_true', help="Will send an email when finished if this option is chosen")
	...
	jobManager = JobManager(options.software_dir, options.sample_dir, options.job_filters, options.qc_settings, options.email)
